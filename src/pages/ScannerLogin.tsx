...
const ScannerLogin: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      showSuccess('Login Successful', 'Welcome to QR Scanner Module');
    } catch (error) {
      showError('Login Failed', 'Invalid credentials or access denied');
    } finally {
      setLoading(false);
    }
  };
        ...
        <div className="bg-white rounded-3xl shadow-2xl p-8">
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <motion.button
              ...
              whileTap={{ scale: 0.98 }}
              className="w-full bg-gradient-to-r from-blue-600 to-purple-600 hover:from-blue-700 hover:to-purple-700 text-white font-semibold py-4 px-4 rounded-xl transition-all duration-200 disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center space-x-2"
            >
              <Camera size={20} />
              <span>{loading ? 'Authenticating...' : 'Access Scanner'}</span>
            </motion.button>
          </form>

          <div className="mt-8 pt-6 border-t border-gray-200 text-center">
            <p className="text-sm text-gray-600">
              Scanner access only • Authorized personnel
            </p>
          </div>
        </div>
      </motion.div>
